...
schools_gdf = gpd.GeoDataFrame({
    'School': ['Dow High School', 'Midland High School'],
    'geometry': [Point(dow_high_coords), Point(midland_high_coords)]
}, crs="EPSG:4326")
...
trip_data_school1 = pd.read_excel("School1Groups.xlsx")
house_location_mapping = trip_data_school1[['ID', 'House Location']].drop_duplicates().set_index('ID')['House Location'].to_dict()
unique_ids = trip_data_school1['ID'].unique()
...
# first plot
plt.title("Midland Schools and Student Homes on OSM")
ax.set_axis_off()
plt.show()
...
schools_gdf.plot(ax=ax, color='orange', marker='x', markersize=100, label='Schools')

# add labels, titles, etc.
plt.title("Midland Road Network with Sections and Student Homes")
plt.legend(title='Legend', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
ox.save_graphml(graph, 'Midland_road_network1.graphml')
plt.show()